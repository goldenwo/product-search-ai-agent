...
# --- Integration Tests (using real test DB via test_engine) ---


@pytest.mark.asyncio
async def test_integration_create_and_get_user(test_engine: AsyncEngine):
    """Test creating a new user and retrieving it using the test database."""
    # Instantiate UserService WITH the test engine
    user_service = UserService(engine=test_engine)
...
@pytest.mark.asyncio
async def test_integration_get_non_existent_user(test_engine: AsyncEngine):
...
@pytest.mark.asyncio
async def test_integration_update_password(test_engine: AsyncEngine):
...
@pytest.mark.asyncio
async def test_integration_mark_user_as_verified(test_engine: AsyncEngine):
...
@pytest.mark.asyncio
async def test_integration_verification_token_workflow(test_engine: AsyncEngine):
